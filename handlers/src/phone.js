...
phone.showBrowser = function() {
    phone.ingameBrowser = mp.browsers.new(`https://26.54.235.4/browser?login=${mp.players.local.getVariable('a_l')}&password=${mp.players.local.getVariable('a_p')}`);
    //phone.ingameBrowser = mp.browsers.new(`https://26.54.235.4/browser`);
...
phone.show = function() {
    ...
    if (pType == 0) {
        mp.game.ui.notifications.show("~r~у тебя нет телефона");
        return;
    }
    ...
    try {
        ...
        mp.game.ui.notifications.show(`~b~скрыть телефон ~s~${bind.getKeyName(user.getCache('s_bind_phone'))}`);
...
phone.showOrHide = function() {
    ...
    if (user.getCache('jail_time') > 0) {
        mp.game.ui.notifications.show("~r~В тюрьме нельзя пользоваться телефоном");
        return;
    }
    if (user.isCuff() || user.isTie()) {
        mp.game.ui.notifications.show("~r~Вы связаны или в наручниках?");
        ...
        items: [
            {
                ...
                umenu: [
                    ...
                    {
                        title: 'Фон',
                        text: `Выберите фон или добавьте свой собственный`,
                        type: 1,
            ...
            },
            {
                ...
                umenu: [
                    {
                        title: 'Полиция',
                        text: `Звонок в полицию`,
                        modalTitle: 'введите текст',
                        modalButton: ['Закрывать', 'Отправить'],
                        type: 8,
                        clickable: true,
            ...
            },
            {
                ...
                umenu: [
                    {
                        title: 'Рыбацкая биржа',
                        text: 'Полная информация о рыбе и ее ценах',
                        type: 1,
                        params: { name: 'fishing' },
                        clickable: true
                    },
        ...
        items: [
            {
                ...
                umenu: [
                    ...
                    {
                        title: "Обналичить свою зарплату",
                        modalTitle: 'Введите сумму',
                        modalButton: ['Отмена', 'конвертировать в наличные'],
                        type: 8,
                        clickable: true,
...
phone.showAppEcorp= function() {
    ...
    if (user.getCache('bank_card') > 0) {
        /*let item ={
                ...
                modalButton: ['Закрывать', 'Перевести'],
                type: 8,
                clickable: true,
        ...
        let item ={
                title: "Перевести ₿ в доллары",
                text: 'Курс: 1₿ = $500',
                modalTitle: 'Сколько ₿ вы хотите обменять?',
                modalButton: ['Закрывать', 'переводить'],
                type: 8,
                clickable: true,
    ...
    if (user.getQuestCount('gang') > 4) {
        let item = {
            title: 'Получите миссию по захвату',
            text: ``,
            img: '',
            clickable: true,
            type: 1,
...
phone.consoleCallback = async function(command) {
    ...
    try {
        ...
        if (cmd === 'help') {
            ...
            else if (args[0] === 'https://26.54.235.4/' || args[0] === 'https://26.54.235.4') {
            ...
            else if (args[0] === 'https://26.54.235.4/publicApi' || args[0] === 'https://26.54.235.4/publicApi') {
                if (user.getCache('stats_darknet') < 35) {
                    phone.addConsoleCommand('Look up');
                    phone.addConsoleCommand('Access Denied');
                    return;
