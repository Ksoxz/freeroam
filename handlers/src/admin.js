...
admin.noClip = function(enable) {
    try {
        ...
        if (noClipEnabled)
            mp.game.ui.notifications.show(`Нажмите H, чтобы выключить No Clip`);
...
admin.godmode = function(enable, notify = true) {
    try {
        ...
        {
            if (godmodeEnabled)
                mp.game.ui.notifications.show(`~q~Режим Бога активирован`);
            else
                mp.game.ui.notifications.show(`~q~Режим Бога отключен`);
        }
